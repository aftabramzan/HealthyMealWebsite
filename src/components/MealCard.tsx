...
export function MealCard({ meal }: { meal: Meal }) {
  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }} className="h-full">
      <Pressable onPress={() => goToSection('contact')} onPressIn={() => animateScale(0.98)} onPressOut={() => animateScale(1)} accessibilityRole="button" className="h-full overflow-hidden rounded-2xl border border-gray-200 bg-white active:opacity-80 md:hover:-translate-y-1 md:hover:shadow-xl">
        ...
        <Text className="min-h-[48px] font-display text-xl leading-6 text-ink">{meal.title}</Text>
        <Text className="mt-2 min-h-[60px] text-sm leading-5 text-gray-500">{meal.description}</Text>
